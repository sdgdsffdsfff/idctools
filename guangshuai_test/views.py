...
def once_check(request):
	...
	for i in result_list:
		if i.device_flag == "snmpwalk failed":
			false_ip.append(i.ip)
			false_dict[i.ip] = "snmpwalk failed"
		else:
			i.start()
	for i in result_list:
		if i.device_flag == "snmpwalk failed":
	...
	if 2 in action:
		...
		if int_err_dict != {}:
			int_err_table = create_int_err_table(int_err_dict,ip_for_create_table)
	...
	if 4 in action:
		#self.count_module_type()
 		#module_number_table = create_table
		pass
	if 5 in action:
		#self.show_sn()
		pass
